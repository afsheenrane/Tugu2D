...
public class Circle extends Shape {
	...
	public Vec2D[] getAABBbounds(){
		return new Vec2D[]{
			new Vec2D(points[0].getX() - radius, points[0].getY() - radius),  //bottom left (conventional cartesian)
	...
	public String repr(){
		...
		return String.format("new Circle(new Vec2D(%s), %.2f);", points[0].getX() + ", " + points[0].getY(), radius);
	}
	...
	public void draw(Graphics2D g2d, double alpha) {
		...
		g2d.drawString(centerOfMass + "", (int)centerOfMass.getX() - 35, (int)centerOfMass.getY() + 13);
		
		g2d.fillOval((int)Math.round(((points[0].getX() - 1) * alpha) + ((prevPos.get(0).getX() - 1) * (1.0 - alpha))),
				(int)Math.round(((points[0].getY() - 1) * alpha) + ((prevPos.get(0).getY() - 1) * (1.0 - alpha))),
				3, 3);
	...
	/**
	 * Draw this circle with lines representing it maximum and minimum extents in the x and y axes.
	 * @param g2d
	 * @param delta
	 */
