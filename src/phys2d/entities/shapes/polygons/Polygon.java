...
public class Polygon extends Shape{
	...
	/**
	 * Create a new polygon defined by points. With no rotation and 10 units mass.
	...
	/**
	 * Create a new polygon defined by points. With rotation of angle radians and mass units of mass.
	 * @param points the edges of the unrotated polygon
	 ...
	 * 
	 * @param points the edges of the unrotated polygon
	 * @param centerOfMass a precomputed center of mass of this polygon. (Also known as the centroid).
	 * @param angle radians by which to rotate the polygon
	 * @param mass mass of the polygon
	 */
	public Polygon(Vec2D[] points, Vec2D centerOfMass, double angle, Material material){
		...
		this.purePoly = generatePurePoly(); //generate the pure poly before rotation is applied.
	 ...
	 * @return a reference polygon centered on the origin. Which is a copy of the current polygon
	 */
	 ...
	 * @param updateRate the rate at which the physics engine updates. This is used
	 ...
	 * @return the minimum and maximum edges of the polygon (in that order) in reference to ref. 
	 */
	@Override
	...
	public void move(double dt){		//TODO
		...
		/*
		  //VELOCITY VERLET
		Vec2D posIncrem = Vec2D.add(Vec2D.getScaled(velocity, dt), Vec2D.getScaled(lastAccel, 0.5 * dt * dt));
		translate(posIncrem);
		
		Vec2D newAccel = Vec2D.getScaled(netForce, invMass);
		
		Vec2D avgAccel = Vec2D.getScaled(Vec2D.add(lastAccel, newAccel), 0.5);
	...
	public static Vec2D[] arrangePoints(Vec2D[] pts){
		...
		if (pts.length > 1){
			...
			//Need this "temp" vector because right now all vectors originate at
			//origin (top left), therefore, all +ve vectors will be pointing downwards
			Vec2D relVec;
			...
			for(Vec2D p : xSorted){  //foreach p in xSorted
				...
				if(p != leftmost && p != rightmost){  //using != instead of .equals because we only want to ignore exactly the left/right points. Not ignore dupes.
				//if( !p.equals(leftmost) && !p.equals(rightmost)){ //exclude the points of interest

					if(relVec.dotProduct(divAxisNorm) >= 0) //positive dot product of the normal = points upwards
						upper.add(p);
					else
						lower.add(p);
				}
	...
	/**
	 * Return the min and max points of the AABB from which the vertices can be extracted.
	 * @param pts
	 * @return
	 */
	...
	/**
	 * Return the min and max points of the shape's AABB swept along its velocity vector for 1 update
	 * @return
	 */
	@Override
	 ...
	 * @param g2d
	 * @param alpha
	 * @param mode 0 = polygon with centroid and points. 1 = include AABB bounds 2 = outline only with points of interest
	 */
...
/*
		...
		//g2d.fillOval((int)points[0].getX() - 1, (int)points[0].getY() - 1, 3, 3);

		if(mode != 2){
			//g2d.drawString(centerOfMass + "", (int)centerOfMass.getX() - 30, (int)centerOfMass.getY());
			for(Vec2D p : points){
				//g2d.drawString(p + "", (int)p.getX() - 30, (int)p.getY() + 12);
				g2d.fillOval((int)((p.getX() - 1)  + (velocity.getX() * alpha)), (int)((p.getY() - 1)  + (velocity.getY() * alpha)), 3, 3);
			}
		}
*/
		//if mode 1, draw AABB
	...
	/**
	 * Return a more or less accurate string representation of this polygon in its current form. 
	...
	public String repr(){
		...
		return String.format("new Polygon(new Vec2D[]{%s}, %.2f);", ptsRepr, mass);
	}
