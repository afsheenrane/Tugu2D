...
public class Vec2D {
	...
	/**
	 * Return the length of the vector before an expensive sqrt operation is carried
	 * out. This can be used to compare vector magnitudes relative to each other
	...
	/**
	 * Is useful for checking if a point is to the left or right of a given vector.
	 * left being positive, right being negative.
	 * @param v1 aka 'a'
	 * @param v2 aka 'b'
	 * @return a X b
	 */
	...
	public void drawVector(Graphics2D g2d){
		...
		g2d.drawString(String.format("[%.2f, %.2f]", x,y), (int)x - 20, (int)y - 10);
	...
	public boolean equals(Object obj) {
		...
		if ((Double.doubleToLongBits(x) != Double.doubleToLongBits(other.x)) && (x != other.x))  //also handles -0 == +0. Which is true
			return false;
		if ((Double.doubleToLongBits(y) != Double.doubleToLongBits(other.y)) && (y != other.y))
			return false;
		return true;
	}
