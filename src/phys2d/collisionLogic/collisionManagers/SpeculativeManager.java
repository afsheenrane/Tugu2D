...
public final class SpeculativeManager extends CollisionManager{
	...
	private void checkAndResolveCollisions(Shape[] shapes){
		...
		collisionTree = new SweptBSPTree(new Rectangle(new Vec2D(500,500), Phys2DMain.XRES + 50, Phys2DMain.YRES + 50), BSPTree.HORIZONTAL_SPLIT, 1, dt);
		...
		for(Shape[] group : collisionGroups){
			...
			for(int i = 0; i < group.length; i++){  //perform a "brute force" collision check
	...
	private void computeForce(Shape s1, Shape s2, double relNormSpeed, Vec2D dispUnit) {
		System.out.println("doing a full resolution*************************************************************************");
		System.out.println(s1 + "\n" + s2);
		System.out.println("dispUnit: " + dispUnit);
		
		//the speed that needs to be removed from the  to ensure no discrete collision
	...
	private void computeShift(Shape s1, Shape s2, Vec2D disp, Vec2D dispUnit) {
		//remove the speeds from the two object. (Use a weighted average of the masses or speed or something))
		...
		s2ratio = 1.0 - s1ratio;  //Doing this to reduce numerical errors from division.
	...
	private void resolveCollision(Shape s1, Shape s2){
		...
		if (CollisionCheckerGJKEPA.isCollidingGJK(s1, s2)) {
		...
		for(int i = 0; i < 2; i++){
			...
			if(isColliding){
				...
				disp.negate();  //Doing this because the disp would originally have been reversed since the shapes were colliding
			}
		...
		//	System.out.println("dispLen: " + disp.getLength());
			...
			Vec2D relVel = Vec2D.sub(s1.getVelocity(), s2.getVelocity());  //making s2 stationary
			relVel.scaleBy(dt);
			//System.out.println("relVel: " + relVel);
			
			double relNormSpeed = relVel.dotProduct(dispUnit);  //relative speed along collision normal
		//	System.out.println("relNormSpeed: " + relNormSpeed);
		//	System.out.println("i: " + i);
			//if a collision is imminent next frame. That is, if the relative normal velocity (px/dt) is greater than the distance between the objects
			//Unless ofcourse a collision has already happened, then immediate resolve the collision.
			if(i == 0){
				if(relNormSpeed >= disp.getLength() || isColliding) {
					...
					//Because the unsticker will already keep them just touching. Dont overcompensate. 
					if(!isColliding)  
		...
		//System.out.println("\nColliders:\n" + s1 + " +\n" + s2 + "\n");
		for(int i = 0; i < 2; i++){
			Vec2D disp = CollisionCheckerGJKEPA.getDisplacementBetweenShapes(s1, s2).getNegated();
		//	System.out.println("\ndisp: " + disp);
		//	System.out.println("dispLen: " + disp.getLength());
		...
		//	System.out.println("i: " + i);
			//if a collision is imminent next frame. That is, if the relative normal velocity (px/dt) is greater than the distance between the objects
			if(i == 0){
				if(relNormSpeed >= disp.getLength()) {
					System.out.println("disp: " + disp);
					computeForce(s1, s2, relNormSpeed, dispUnit);
					computeShift(s1, s2, disp, dispUnit);	
				}
	...
	private void unstickShapes(Shape s1, Shape s2, Vec2D resolution){
		...
		//this second scale decides the direction of the translation (positive or negative)
