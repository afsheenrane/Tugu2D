...
/**
 * A collection of methods that utilizes the GJK and EPA algorithms to solve problems such
 * as getting collision normals between shapes and finding the closest displacement between them.
 * @author afsheen
 */
...
public final class CollisionCheckerGJKEPA {
	 ...
	 */
	public static boolean isCollidingGJK(Shape s1, Shape s2){
	...
	/**
	 * Return the vector by which s2 needs to be translated, so as not to collide with s1 <br>
	...
	private static ArrayList<Vec2D> computeGJKsimplex(Shape s1, Shape s2){
		...
		while(true){
			...
			/*
			//The following condition was created to combat infinite loops generated by float point errors
			//when the simplex was extremely close to the origin. Since in the worst case there is an
			//extremely small penetration (negligible), the safer alternative of just returning false is taken.
			if(!checkedPts.add(supp)){  //If the set contained the value already, return false.
				//System.out.println("simplex: " + simplex);
				//System.out.println("bypass used");
				//simplex.remove(0); 
				//return simplex;
			}
			*/
			
			//The above is no longer needed because of the addition of a tolerance
			//to the getLeftOrRight method.
			
			//if the point isnt past the Vec2D.ORIGIN, then the mink diff cannot contain the Vec2D.ORIGIN
			...
			if(simplex.size() == 3 && LinePolyTools.isPointInPolygon(new Polygon(simplex.toArray(new Vec2D[]{})), Vec2D.ORIGIN))
				return simplex;  //if it is in the simplex, then collision
			
			
			//if it does not contain the Vec2D.ORIGIN, find the new search direction				
			Vec2D[] pts = Polygon.arrangePoints(simplex.toArray(new Vec2D[]{}));
								
			
			//Simplex is a line. Find if the origin is to the left or the right.
			if(pts.length == 2){
				if(LinePolyTools.getLeftOrRight(pts[0], pts[1], Vec2D.ORIGIN) > 0)
					dir = Vec2D.sub(pts[1], pts[0]).getNormal();
				else
					dir = Vec2D.getNegated(Vec2D.sub(pts[1], pts[0]).getNormal());
					
			}
			
			//The simplex is a triangle. Find the voronoi region in which the ORIGIN lies.
			else{
	...
	/**
	 * Helper for GJK related functions. Takes in a triangular simplex, an 
	 * array containing the simplex points in clockwise order and the current direction.
	 * @param pts sorted simplex points (clockwise)
	 * @param simplex the gjk simplex
	 * @param dir current gjk search direction
	 */
	private static void removeInvalidPointAndSetDir(Vec2D[] pts, ArrayList<Vec2D> simplex, Vec2D dir) {
		if(LinePolyTools.getLeftOrRight(pts[0], pts[1], Vec2D.ORIGIN) > 0){
			dir = Vec2D.sub(pts[1], pts[0]).getNormal();
	 ...
	 * @return
	 */
	private static Vec2D getCollisionResolutionEPA(Shape s1, Shape s2, ArrayList<Vec2D> simplex){
		...
		while(true){  //Begin the EPA algorithm
			...
			Vec2D[] closestEdge = new Vec2D[]{simplex.get(simplex.size() - 1), simplex.get(0)};
			Vec2D closestDisp = LinePolyTools.ptToLineDisp(Vec2D.ORIGIN, closestEdge);  //Compare this with ln202 (after "computing") and see if that is redundant op
			//was using ptToLineSegDisp but since the origin is inside the simplex, the edge cases
			...
			//Upon completion, we should have all the information of the closest edge of the mink diff to the origin
			Vec2D edgeNorm = closestDisp.getNormalized();
			Vec2D point = support(s1, s2, edgeNorm);
			
			double newPtDistFromOrigin = point.dotProduct(edgeNorm);  //scalar projection
			
			//If our new expansion point is already on the edge (or very close to) then return that edge normal
	...
	/**
	 * Return a point which is guaranteed to an edge of the minkowski difference of p1 and p2. 
	 * Along the dir axis. This is used to build the simplex to be used in GJK
	 * @param s1
	 * @param s2
	 * @param dir
	 * @return
	...
	/**
	 * I actually dont remember why I coded this. But i think it is something to do with swept
	 * collision detection to get the closest displacement to check if the relvel will 
	 * cause a collsion in the next frame.
	 * @param s1
	 * @param s2
	 * @return
	 */
	public static Vec2D getDisplacementBetweenShapes(Shape s1, Shape s2){
		...
		Vec2D[] simplex = new Vec2D[2];  //simplex will only be a line. Hence forcing it to a size 2 array
		Vec2D dir;
	...
	public static Vec2D getDisplacementBetweenShapes(Shape s1, Shape s2, Vec2D[] simplex, Vec2D dir){
		final double TOLERANCE = 0.1;
		...
		while(true){
			...
			x++;
			if(x > failureTol) {
				System.out.println("DISPLACEMENT BETWEEN SHAPES Checker failure");
			...
			//see if the new support point is actually making progress towards the origin
			if(Vec2D.sub(supp, simplex[0]).dotProduct(dir) <= TOLERANCE){
				return closestPt;  //the closest feature of the line
			}
			
			if(simplex[0].getSquaredLength() < simplex[1].getSquaredLength())  //Remove the further point from the origin
				simplex[1] = supp;
			else
				simplex[0] = supp;
		...
		while(true){
			...
			if(simplex.get(simplex.size() - 1).dotProduct(dir) < 0){  
				...
				if(simplex.size() != 2){
					...
					//If triangular simplex, remove the point not contributing to
					//the correct voronoi region containing the origin.
					removeInvalidPointAndSetDir(Polygon.arrangePoints(simplex.toArray(new Vec2D[]{})), simplex, dir);
					
				}
				//Since no explicit collision, use the swept solver.
				return new Object[]{getDisplacementBetweenShapes(s1, s2, simplex.toArray(new Vec2D[]{}), dir), false};  
			}
			...
			if(simplex.size() == 3 && LinePolyTools.isPointInPolygon(new Polygon(simplex.toArray(new Vec2D[]{})), Vec2D.ORIGIN)){
				...
				return new Object[]{resolution, resolution.equals(Vec2D.ORIGIN) ? false : true};
			}
