...
public class BSPTree {
	...
	protected static int DEPTH_CAP = 20;  //TODO make protected from public
	
	protected int depth = 0;
	
	protected final List<Shape> items;
	protected final Rectangle bounds;
	protected final BSPTree[] children;
	...
	public static void calculateDepthCap(ArrayList<Shape> shapes){
		...
		for (Shape s : shapes){
			if(!(s instanceof WorldBound)){
				...
				t += Math.abs(aabb[0].getX() - aabb[aabb.length - 1].getX());  //abs of min - max			
	...
	public void split(){
		...
		if(splitMode == VERTICAL_SPLIT){
			...
			newBounds = new Rectangle(com.getCopy(), bounds.getLength() / 2, bounds.getHeight());  //doing com copy because com is being aliased.
																								   //not copying newbounds because it is being being re-assigned
			c0 = new BSPTree(newBounds, splitMode * -1, depth + 1);  //left rect
			
			com.setX(com.getX() + bounds.getLength() / 2);
			newBounds = new Rectangle(com.getCopy(), bounds.getLength() / 2, bounds.getHeight());
			c1 = new BSPTree(newBounds, splitMode * -1, depth + 1);  //right rect
		}
		else{
			
			com.setY(com.getY() - (bounds.getHeight() / 4));
			newBounds = new Rectangle(com.getCopy(), bounds.getLength(), bounds.getHeight() / 2);
	...
	/**
	 * Get which side the Shape s can be inserted on. 0 = left, 1 = right, -1 = both
	 * @param s
		...
		if(items.size() > MAX_ITEMS && depth < DEPTH_CAP){
			...
			for(int i = items.size() - 1; i >= 0; i--){  //items size is not guaranteed 
				...
				if(insertionSide != -1){  //if a valid insertion side is found, insert the item into that side
					...
					children[1].insert(items.remove(i));  //both do the same thing.
					//im just saving a line by doing the removal at the same time.
				}
			}
		}
	}
	...
	protected ArrayList<Shape> getPossibleCollidersHelper(Shape s, ArrayList<Shape> colliders){
		int shapeSide = getInsertionSide(s);  //Get which side the shape fits into

		//if the shape can fit on a specific side, and if there are children, go deeper into tree to find matches
		if(children[0] != null){
			if(shapeSide != -1)
		...
		//Now add everything at this current level. The above statement would ensure the depths of the tree have been traversed
	...
	public void draw(Graphics2D g2d){
		...
		g2d.drawString(depth + " " + items.size(),(int) com.getX() - 10,(int)(com.getY() + 5));
		if(children[0] != null){
			if(splitMode == VERTICAL_SPLIT)
				g2d.drawLine((int)com.getX(),(int)(com.getY() - (bounds.getHeight() / 2)),(int)com.getX(),(int)(com.getY() + (bounds.getHeight() / 2)));
			
			else
				g2d.drawLine((int)(com.getX() - (bounds.getLength() / 2)),(int)com.getY(), (int)(com.getX() + (bounds.getLength() / 2)),(int)com.getY());
