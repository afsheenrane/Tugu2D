...
public final class LinePolyTools {
    ...
    /**
     * Return the displacement to the closest side of the polygon when p lies
     * inside the polygon.
     ...
     * @return the exact displacement vector the point is from the closest side
     *         of the polygon.
     ...
     * @param p
     * @param mode
     * @return if mode is 0, return an empty vec2d array if the points is not in
     *         poly. Otherwise a populated vec2D[]
     *         populated = true
     *         empty = false
     *         If mode is 1, return the displacement vector of the point to the
     *         closest polygon edge
     */
    private static Vec2D[] pointInPolyTester(Polygon poly, Vec2D p, int mode) {
        ...
        for (int i = 0; i < polyPts.length - 1; i++) {
            triangle = new Polygon(
                    new Vec2D[] { polyPts[i], poly.getCOM(), polyPts[i + 1] },
                    0);
            ...
            if (subTriPts.size() == 3) { // if a valid voronoi region is found
                if (mode == 0) // if boolean answer request, just return "true"
                    return TRUE;
                else if (mode == 1) { // otherwise compute the length from the
                                      // polygon side
                    subTriPts.remove(poly.getCOM()); // remaining two vectors
                                                     // form line
                    return new Vec2D[] { ptToLineDisp(p, new Vec2D[] {
                            subTriPts.get(0), subTriPts.get(1) }) };
                }
                else
                    System.out.println(
                            "FLOATING POINT ERROR IN PHYSTOOLKIT. FIX ME");
            }
        }
        // for the final pts. Where the last point in polygon wraps to the first
        triangle = new Polygon(new Vec2D[] { polyPts[polyPts.length - 1],
                poly.getCOM(), polyPts[0] }, 0);

        if (isPointInTriangle(triangle, p).size() == 3) { // if the point is in
                                                          // the final region
            if (mode == 0) // bool requested
                return TRUE;
            else // exact dist requested
                return new Vec2D[] { ptToLineDisp(p, new Vec2D[] { polyPts[0],
                        polyPts[polyPts.length - 1] }) };
        }
        else // if it is not in the final region return that the point is not in
             // the polygon
            return FALSE;
    }
     ...
     * @param p the point to check
     * @return an empty arraylist if the point is not contained, or an arraylist
     *         of size 3
     *         if it is.
     */
    private static ArrayList<Vec2D> isPointInTriangle(Polygon triangle,
            Vec2D p) {
        ...
        return new ArrayList<Vec2D>(Arrays.asList(triPts)); // return the
                                                            // triangle points
                                                            // if it is
                                                            // contained
    }
    ...
    public static Vec2D ptToLineDisp(Vec2D p, Vec2D[] line) {
        ...
        // Assuming cw order of points has been conserved throughout
        // calculations.
    ...
    /**
     * Return the displacement vector of a point 'p' from the line segment
     * lineSeg.
     * ie. keeping the lineSeg as our reference frame, what is the displacement
     * of p
     * from it?
     * 
     * @param p the point to check
     ...
     * Not restricted to vertices. It is the true closest point anywhere on the
     * line.
     * 
     * @param lineSeg the line segment to get the closest point from
     * @return the point on lineSeg closest to the origin.
     */
    public static Vec2D getClosestPtToOrigin(Vec2D[] lineSeg) {
        ...
        else if (proj * proj >= lineSegVec.getSquaredLength()) // if its past
                                                               // the 1 point
            return lineSeg[lineSeg.length - 1];

        else
            return Vec2D.add(Vec2D.getScaled(lineSegVec.getNormalized(), proj),
                    lineSeg[0]);
    ...
    public static Vec2D getClosestPtToOrigin(ArrayList<Vec2D> lineSeg) {
        ...
        if (lineSeg.size() == 1)
            return lineSeg.get(0);

        Vec2D lineSegVec = Vec2D.sub(lineSeg.get(lineSeg.size() - 1),
                lineSeg.get(0));
        Vec2D relOrigin = Vec2D.getNegated(lineSeg.get(0));
        ...
        if (proj <= 0) // if its before the 0 point
            return lineSeg.get(0);
                                                               ...
                                                               // the 1 point
            return lineSeg.get(lineSeg.size() - 1);
        else
            return Vec2D.add(Vec2D.getScaled(lineSegVec.getNormalized(), proj),
                    lineSeg.get(0));
    ...
    public static int getLeftOrRight(Vec2D v1, Vec2D v2, Vec2D p) {
        ...
        return (result > 0) ? 1 : -1; // return 1 if result is positive(left),
                                      // -1 otherwise (right). was >=
    }
    ...
    public static Polygon generateConvexHull(Vec2D[] points) {
        ...
        ArrayList<Vec2D> remainingPoints = new ArrayList<Vec2D>(
        ...
        for (Vec2D pt : remainingPoints) {
            // if(Vec2D.perpDotProduct(Vec2D.sub(xMM[1], xMM[0]), Vec2D.sub(pt,
            // xMM[0])) >= 0)
        ...
        genHull(xMM[0], xMM[1], leftPts, hullPoints); // gen hull from left
                                                      // points
        genHull(xMM[1], xMM[0], rightPts, hullPoints); // gen hull from right
                                                       // points
     ...
     * @param hull
     */
    private static void genHull(Vec2D p1, Vec2D p2, ArrayList<Vec2D> pointSet,
        ...
        Vec2D relVec;
        Vec2D furthestPt = new Vec2D(Double.NEGATIVE_INFINITY,
    ...
    /**
     * Depending on the mode, produces the minkowski sum or difference of the
     * two polygons
     * 
    ...
    private static Circle circSumDif(Circle c1, Circle c2, int mode) {
        Vec2D center = (mode > 0)
                ? Vec2D.add(c1.getPoints()[0], c2.getPoints()[0])
                : Vec2D.sub(c1.getPoints()[0], c2.getPoints()[0]);
     ...
     * @param p1 first polygon
     * @param p2 second polygon
     * @return the polygon which is the minkowski difference of p1 and p2. (p1 -
     *         p2)
     */
    public static Polygon polyDifference(Shape s1, Shape s2) {
        Polygon p1, p2;
        try {
            p1 = (Polygon) s1;
        }
        catch (ClassCastException e) {
            p1 = new Polygon(((Circle) s1).getFullAABBPts());
        }
        try {
            p2 = (Polygon) s2;
        }
        catch (ClassCastException e) {
            p2 = new Polygon(((Circle) s2).getFullAABBPts());
        }
     ...
     * @param c1 the first circle
     * @param c2 the second circle
     * @return the circle which is the minkowski difference of c1 and c2. (c1 -
     *         c2)
     */
