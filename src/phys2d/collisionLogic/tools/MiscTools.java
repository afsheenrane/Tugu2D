...
public final class MiscTools {
	...
	/**
	 * From the set of points return the minimum and maximum points along the ref axis
	...
	public static Vec2D getMin(Vec2D[] points, Vec2D ref){
		...
		for(Vec2D p : points){
			...
			/*
			 *if they have the same projection along the reference axis, sort them
			 *by their projection in the reference normal. 
			 *eg: If ref is the x-axis, then the min vector will be the vector with
			 *the lowest x and y extents.
			 */
			else if(refDot == minDot){
				if(min.dotProduct(refNorm) > p.dotProduct(refNorm))
					min = p;  //no need to make minDot = refDot, because they're already equal.
			}
		}
		return min;
	}	
	...
	/**
	 * Generate a number of random vectors within the floor and ceiling parameters
	 ...
	 * @return a vector array of size count containing randomly generated vectors
